...
from vapp.AsteriskManager import AsteriskManager

config = Config()
manager = None

def startManager(logger):
    global manager
    manager = AsteriskManager(logger = logger, username = config.managerUser(), password = config.managerPassword())
    manager.start()
